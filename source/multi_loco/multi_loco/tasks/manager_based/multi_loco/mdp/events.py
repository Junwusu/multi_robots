...
def hide_inactive_robot(
    ...
    if quad_ids.numel() > 0:
        ...
        _set_root_to_hidden(hexa, quad_ids, hidden_pos_t)

    # inactive in biped envs: quad/hex should be hidden
    if biped_ids.numel() > 0:
        _set_root_to_hidden(quad, biped_ids, hidden_pos_t)
        _set_root_to_hidden(hexa, biped_ids, hidden_pos_t)

    # inactive in hex envs: biped/quad should be hidden
    if hex_ids.numel() > 0:
        _set_root_to_hidden(biped, hex_ids, hidden_pos_t)
        _set_root_to_hidden(quad, hex_ids, hidden_pos_t)
